...
*   **SKILL AUTOMÁTICO 1 - AUTO-COMMIT:** Siempre que realice un cambio, implementación o modificación de código que cumpla un objetivo, debo subir todo a Git de forma automática con un commit descriptivo siguiendo Conventional Commits.
*   **SKILL AUTOMÁTICO 2 - SINCRONIZACIÓN DE MONOREPO (Full-Stack Sync):** Dado que este es un ecosistema de 3 partes (Backend `server.js`, App Android `Kotlin`, Admin Web `React/TS`), **siempre** que modifique una entidad base o un endpoint en el backend, debo automáticamente proponer o ejecutar la actualización de los DTOs en Android y las interfaces TypeScript en Web. El sistema nunca debe quedar desincronizado.
*   **SKILL AUTOMÁTICO 3 - ANTICIPACIÓN FUNCIONAL:** No me limitaré a programar el "Happy Path" (camino feliz). Siempre que construya una funcionalidad, identificaré e implementaré el manejo de estados vacíos, errores de red, o casos límite (edge cases) por defecto, para construir un producto robusto e intuitivo desde el inicio.
*   **SKILL AUTOMÁTICO 4 - AUTO-PUSH (Cloud Sync):** Después de cada commit realizado con éxito (SKILL 1), debo ejecutar inmediatamente un `git push origin [rama_actual]` para asegurar que el trabajo esté siempre respaldado en la nube y sincronizado con el repositorio remoto.
